...
| **The Old Ring** | (≈283, 247) plateau behind the sea cliffs | Neolithic stone circle: 8 mossy menhirs (one fallen outside the ring, one snapped into stump + leaning capstone), a broken two-slab altar in the middle. A turf barrow with a stone ring + leaning sentinel (≈303, 285) and scattered fallen slabs nearby. Stones are fbm-wobbled cylinders with vertex-coloured weathering (damp base, sun-bleached crowns, moss, lichen) — weathered monoliths, not blocks. Far older than the city. Out in the grove, more megaliths each on their own: a dolmen with tilted capstone (≈258, 214), a small cairn (≈315, 214), a weathered leaning monolith (≈252, 297), a falling alignment of three slabs (≈260, 303), a half-buried stub, and a pair with the fallen one between them. The two far-hill slabs that used to squat at the NE map edge are skipped instead; the sector is now a dark evergreen grove much thicker than the hill forest, with the ring, barrow and dolmen in open clearings. |
| **Watchtower** | (199, 268) on the cliff edge above the cove | A ruined medieval stone keep — far younger than the ring. The sea-facing wall is the tallest survivor (~10 m) with window slits; the back corners stand low, the SW corner has collapsed out over the cliff lip (its slabs straddle the lip with their bases embedded in the rock — the cliff face drops sharply a few metres west of the tower), the crowns are ragged broken, and rubble is tumbled down the face to the water. Worn grey-brown masonry with fbm-displaced boxes (faint courses, damp bases, mossy crowns); a fallen length of curtain wall behind it and evergreen shrubs at the base. Someone kept a watch over the bay from here once. All rubble stones sit on visible rock — an earlier pass left a few on hidden rock shelves that read as floating blocks from the bay; they're re-embedded on the lower slope, and the rim pines sit on the plateau just inland of the edge, below the crowns. |
...
- **Living landscape** — hero trees that sway, grass tufts that bend, a
  ...
  stone circle, a dolmen, a cairn and scattered megaliths in a dark evergreen
  grove behind the sea cliffs, where a ruined watchtower still keeps a broken
  watch over the bay from the cliff edge.
...
- **Weather** — clear, cloudy, fog, rain (per-drop length/slant variance); fair-weather clouds drift high (y 56–74) and go dark + faint at night, so a moonlit sky never shows pale slabs hovering over the ridge,
