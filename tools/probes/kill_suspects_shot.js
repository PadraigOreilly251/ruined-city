(() => {
  ...
  const box=new THREE.Box3(), sz=new THREE.Vector3(); let killed=[];
  scene.traverse(o=>{
    if(!o.isMesh||!o.visible) return;
    box.setFromObject(o); box.getSize(sz);
    const nearWhite = o.material&&o.material.color&&o.material.color.r>0.9&&o.material.color.g>0.9&&o.material.color.b>0.9;
    if(Math.max(sz.x,sz.z)>60 && nearWhite && o.name!=='__tarnice__'){
      killed.push(`${o.name||'box'} bb=${sz.x.toFixed(0)}x${sz.y.toFixed(0)}x${sz.z.toFixed(0)} base=${box.min.y.toFixed(0)} transparent=${!!o.material.transparent}`);
      o.visible=false;
    }
  });
  return 'KILLED '+killed.length+':\n'+killed.join('\n');
})()
